...
#define MOTOR_HOME_OFFSET 23080  // Encoder steps
#define ENCODER_SCALING_FACTOR 4990   // 23080 encoder steps per 4.625 inches
#define MOTOR_MAX_EXTENT_INCH (4.625-0.5)  // Half inch buffer
#define MOTOR_MAX_EXTENT 20584 //or ((int32_t)(MOTOR_MAX_EXTENT_INCH*ENCODER_SCALING_FACTOR))
...
byte setToutB[] = {0x80, 0x64, 0x00};  // Timeout (100 ms)
...
void loop() {
  ...
  if (!done) {
    done = homeMotors(switches);
    Serial.println(done ? "Done homeing" : "Still homeing");
    int32_t pos = 0;
    for (int i=0; i<NUM_MOTORS; i++) {
    ...
    while (1) {}*/
    float target = 0.0;
    for (int i=0; i<4+0*NUM_MOTORS; i++) {
...
bool setRealPosition(byte motorI, float inches, byte *limitSwitches) {
  ...
  int32_t targetPos = motorEncOffset[motorI] + targetScale;
  int32_t currentPos;
...
bool homeMotors(byte *limitSwitches) {
  ...
  for (int i=0; i<NUM_MOTORS; i++) {
    ...
    if (limitSwitches[i*2+(reverse ? 1 : 0)]) {
      ...
      int32_t pos;
      bool success = false;
      for (int i=0; !(success=readEncoderPos(MOTORS[i], &pos)); i++) {
        if (i >= 5) {
          fatalError();
        }
      }
      motorEncOffset[i] = pos + (reverse ? -MOTOR_HOME_OFFSET : MOTOR_HOME_OFFSET);
    }else {
